  ...
  appData = Storage.update((data) => {
    data.rooms.forEach((room) => {
      const activeBooking = data.bookings.find(
        (b) => b.roomId === room.id && b.status !== 'cancelled' && b.status !== 'checked-out' && b.checkIn <= today && b.checkOut > today
      );
      const upcomingBooking = data.bookings.find(
        (b) => b.roomId === room.id && b.status === 'confirmed' && b.checkIn > today
      );
...
function bindEvents() {
  ...
  // Dining & Services nav links — scroll + close mobile menu
  document.querySelectorAll('a[href="#dining"], a[href="#services"]').forEach((link) => {
  ...
  // Toggle add-item / add-service panels
  document.getElementById('toggleAddMenuItemBtn').addEventListener('click', () => {
    const card = document.getElementById('addMenuItemCard');
...
function renderRoomTypes() {
  ...
  const rooms = Storage.getRooms();
  grid.innerHTML = Object.values(ROOM_TYPES).map((type) => {
    ...
    return `
      <div class="room-type-card">
        ...
        <div class="room-type-body">
          ...
          <div class="room-type-meta"><span>👤 Up to ${type.capacity}</span><span>🛏 ${typeRooms.length} rooms</span></div>
...
function populateRoomTypeSelect() {
  ...
  const options = Object.values(ROOM_TYPES)
    .map((t) => `<option value="${t.id}">${t.name} — ${formatCurrency(t.price)}/night</option>`).join('');
  select.innerHTML = options;
  filterSelect.innerHTML = '<option value="all">All Types</option>' +
    Object.values(ROOM_TYPES).map((t) => `<option value="${t.id}">${t.name}</option>`).join('');
}
...
function getAvailableRoomsForDates(typeId, checkIn, checkOut) {
  ...
  return rooms.filter((room) => {
    const conflicts = bookings.some(
      (b) => b.roomId === room.id && b.status !== 'cancelled' && b.status !== 'checked-out' &&
...
function renderAvailabilityList() {
  ...
  list.innerHTML = Object.values(ROOM_TYPES).map((type) => {
    let count;
    ...
    return `<div class="avail-item"><span>${type.name}</span><span class="avail-count availability-badge ${cls}">${count} rooms</span></div>`;
  }).join('');
}
...
function handleBooking(e) {
  ...
  if (guests > type.capacity) { showToast(`This room type supports up to ${type.capacity} guests`, 'error'); return; }
  const availableRooms = getAvailableRoomsForDates(roomType, checkIn, checkOut);
  if (!availableRooms.length) { showToast('No rooms available for selected dates', 'error'); return; }
  const nights = daysBetween(checkIn, checkOut);
  const total = type.price * nights * 1.12;
  const room = availableRooms[0];
  const code = generateBookingCode();
  const booking = {
    id: 'bk-' + Date.now(), code, guestName, guestEmail, guestPhone,
    roomId: room.id, roomType, checkIn, checkOut, guests,
    total: Math.round(total * 100) / 100, status: 'confirmed', createdAt: new Date().toISOString()
  };
  appData = Storage.update((data) => {
...
function upsertGuest(data, guest) {
  ...
  if (existing) {
    existing.name = guest.name; existing.phone = guest.phone;
    existing.lastVisit = todayStr(); existing.visits = (existing.visits || 1) + 1;
  } else {
    data.guests.push({ id: 'guest-' + Date.now(), name: guest.name, email: guest.email, phone: guest.phone, visits: 1, lastVisit: todayStr() });
  }
}
...
function handleLookup(e) {
  ...
  if (!booking) { result.innerHTML = '<p class="empty-state">No booking found. Please check your code or email.</p>'; return; }
...
function renderDashboard() {
  const rooms = Storage.getRooms();
  const bookings = Storage.getBookings();
  const foodOrders = Storage.getFoodOrders();
  ...
  const pendingFoodOrders = foodOrders.filter((o) => o.status === 'pending').length;
  const pendingServiceReqs = serviceRequests.filter((r) => r.status === 'pending').length;
  ...
  document.getElementById('metricsGrid').innerHTML = `
    ...
    <div class="metric-card"><div class="metric-label">Pending Food Orders</div><div class="metric-value">${pendingFoodOrders}</div></div>
    <div class="metric-card"><div class="metric-label">Pending Service Requests</div><div class="metric-value">${pendingServiceReqs}</div></div>`;

  // Today's activity (bookings)
  const activities = bookings.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5);
  document.getElementById('todayActivity').innerHTML = activities.length
    ? activities.map((b) => `
        <div class="activity-item">
          <span class="activity-dot" style="background:${b.status === 'checked-in' ? 'var(--color-success)' : 'var(--color-info)'}"></span>
          <div><strong>${b.guestName} — ${b.status}</strong><br><span class="text-muted">${b.code} · ${formatDate(b.checkIn)}</span></div>
        </div>`).join('')
  ...
  const recentEl = document.getElementById('recentOrdersActivity');
  if (recentEl) {
    const combined = [
      ...foodOrders.map(o => ({ ...o, _type: 'Food Order' })),
      ...serviceRequests.map(r => ({ ...r, _type: 'Service Request' }))
    ].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5);
    recentEl.innerHTML = combined.length
      ? combined.map((rec) => `
          <div class="activity-item">
            <span class="activity-dot" style="background:${rec.status === 'pending' ? 'var(--color-warning)' : rec.status === 'delivered' || rec.status === 'completed' ? 'var(--color-success)' : 'var(--color-info)'}"></span>
            <div>
              <strong>${rec._type}</strong> — <span class="status-pill ${rec.status.replace('-','')}">${rec.status}</span><br>
              <span class="text-muted">${rec.id} · ${rec.guestName} · Room ${rec.roomNumber} · ${new Date(rec.createdAt).toLocaleString()}</span>
            </div>
          </div>`).join('')
      : '<p class="empty-state">No orders or requests yet.</p>';
  }
}
...
function setRoomStatus(roomId, status) {
  appData = Storage.update((data) => { const room = data.rooms.find((r) => r.id === roomId); if (room) room.status = status; return data; });
  syncRoomStatuses(); renderAdminRooms(); updateHeroStats();
  showToast(`Room status updated to ${status}`, 'info');
}

function showAddRoomModal() {
  const typeOptions = Object.values(ROOM_TYPES).map((t) => `<option value="${t.id}">${t.name}</option>`).join('');
  showModal(`
    <h3>Add New Room</h3>
    <form id="addRoomForm">
      <div class="form-group"><label>Room Number</label><input type="text" id="newRoomNumber" required placeholder="e.g. 105"></div>
      <div class="form-group"><label>Room Type</label><select id="newRoomType" required>${typeOptions}</select></div>
      <div class="form-group"><label>Floor</label><input type="number" id="newRoomFloor" required min="1" max="20" value="1"></div>
  ...
  document.getElementById('addRoomForm').addEventListener('submit', (e) => {
    ...
    if (Storage.getRooms().some((r) => r.number === number)) { showToast('Room number already exists', 'error'); return; }
    appData = Storage.update((data) => { data.rooms.push({ id: 'room-' + Date.now(), number, type, status: 'available', floor }); return data; });
    closeModal(); renderAll(); showToast(`Room ${number} added`, 'success');
  });
}
...
function renderBookingsTable() {
  ...
  if (search) {
    bookings = bookings.filter((b) =>
      b.code.toLowerCase().includes(search) || b.guestName.toLowerCase().includes(search) || b.guestEmail.toLowerCase().includes(search));
  }
...
function checkInBooking(bookingId) {
  ...
  renderAll(); showToast('Guest checked in successfully', 'success');
}
...
function checkOutBooking(bookingId) {
  ...
  syncRoomStatuses(); renderAll(); showToast('Guest checked out. Room is now available.', 'success');
}
...
function cancelBooking(bookingId) {
  ...
  syncRoomStatuses(); renderAll(); showToast('Booking cancelled', 'info');
}
...
function handleQuickCheckIn(e) {
  ...
  if (booking.status !== 'confirmed') { showToast(`Cannot check in — status is ${booking.status}`, 'error'); return; }
...
function handleQuickCheckOut(e) {
  ...
  if (!booking) {
    ...
    if (room) booking = Storage.getBookings().find((b) => b.roomId === room.id && b.status === 'checked-in');
  }
  if (!booking) { showToast('No active booking found', 'error'); return; }
  if (booking.status !== 'checked-in') { showToast(`Cannot check out — status is ${booking.status}`, 'error'); return; }
...
function renderGuests() {
  ...
  if (!guests.length) {
    ...
    Storage.getBookings().forEach((b) => {
      if (!fromBookings[b.guestEmail]) fromBookings[b.guestEmail] = { name: b.guestName, email: b.guestEmail, phone: b.guestPhone, visits: 0 };
      fromBookings[b.guestEmail].visits++;
    });
    const derived = Object.values(fromBookings);
    grid.innerHTML = derived.length
      ? derived.map((g) => `<div class="guest-card"><h4>${g.name}</h4><div class="guest-email">${g.email}</div><div class="guest-meta">${g.phone} · ${g.visits} booking${g.visits > 1 ? 's' : ''}</div></div>`).join('')
      : '<p class="empty-state">No guests yet</p>';
    return;
  }
  grid.innerHTML = guests.map((g) => `
    <div class="guest-card"><h4>${g.name}</h4>
    <div class="guest-email">${g.email}</div>
    <div class="guest-meta">${g.phone} · ${g.visits || 1} visit${(g.visits || 1) > 1 ? 's' : ''} · Last: ${g.lastVisit ? formatDate(g.lastVisit) : '—'}</div></div>`).join('');
}
